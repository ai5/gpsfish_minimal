...
MovePicker::MovePicker(const Position& p, Move ttm, Depth d, const History& h)
  ...
  else
  {
	...
	searchTT = V(ttMoves[0].move = MOVE_NONE);
  }
...
//  Square from;
  ...
  {
      m = cur->move;
      cur->score = H.value(osl::ptypeO(m), move_to(m));
  }
}
...
#if 1
	    std::stable_sort(lastGoodNonCapture, lastMove,
		      [](MoveStack const& a, MoveStack const& b){ return b < a; });
#else
	    std::sort(lastGoodNonCapture, lastMove,
		      [](MoveStack const& a, MoveStack const& b){ return b < a; });
#endif
